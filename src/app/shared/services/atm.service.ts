...
export class AtmService {
 ...
 restock(denomination: number, quantity: number): Observable<void> {
  return new Observable<void>(observer => {
    const currentQuantity = this.denominations.get(denomination) ?? 0;
    const newQuantity = currentQuantity + quantity;
    this.denominations.set(denomination, newQuantity);
